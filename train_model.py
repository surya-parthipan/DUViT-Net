...
class MSDDataset(Dataset):
    def __init__(self, image_paths, mask_paths, modalities=None, slice_axis=2, transform=None,
                 max_slices_per_volume=None, slice_selection_mode='top_k'):
        super().__init__()
        ...
        for idx in range(len(self.image_paths)):
            ...
            if len(relevant_slices) == 0:
                continue  # Skip volumes without relevant slices

            # Apply selective slicing
            if self.max_slices_per_volume is not None:
                if self.slice_selection_mode == 'top_k':
                    # Select slices with the highest sum of mask values
                    slice_indices = relevant_slices
                    slice_values = slice_sums[relevant_slices]
                    top_k_indices = slice_indices[np.argsort(slice_values)[-self.max_slices_per_volume:]]
                    selected_slices = np.sort(top_k_indices)
                elif self.slice_selection_mode == 'random':
                    # Randomly select slices
                    if len(relevant_slices) > self.max_slices_per_volume:
                        selected_slices = np.sort(np.random.choice(
                            relevant_slices, self.max_slices_per_volume, replace=False
                        ))
                    else:
                        selected_slices = relevant_slices
                elif self.slice_selection_mode == 'every_nth':
                    # Select every nth slice
                    n = max(1, len(relevant_slices) // self.max_slices_per_volume)
                    selected_slices = relevant_slices[::n][:self.max_slices_per_volume]
                else:
                    raise ValueError(f"Invalid slice_selection_mode: {self.slice_selection_mode}")
            else:
                selected_slices = relevant_slices

            # Store (volume index, slice index) pairs
            self.indices.extend([(idx, slice_idx) for slice_idx in selected_slices])
...
def train_model(model, dataloaders, criterion, optimizer, num_epochs, device, num_classes, save_path, task, use_albumentations, early_stopping_patience=10):
    best_loss = float('inf')
    scaler = GradScaler()
...
def main(task, use_albumentations):
    ...
    # Train-Val split
    # train_image_paths, val_image_paths, train_mask_paths, val_mask_paths = train_test_split(image_paths, mask_paths, test_size=0.60, random_state=42)
    x_image_paths, xx_image_paths, x_mask_paths, xx_mask_paths = train_test_split(image_paths, mask_paths, test_size=0.80, random_state=42)
    

    train_image_paths, val_image_paths, train_mask_paths, val_mask_paths = train_test_split(
        x_image_paths, x_mask_paths, test_size=0.20, random_state=42
    )

    # Define transforms
    train_transform = get_training_augmentation() if use_albumentations else None

    # Create Datasets with selective slicing
    max_slices_per_volume = 20  # Adjust as needed
    slice_selection_mode = 'top_k'  # Choose from 'top_k', 'random', 'every_nth'
    ...
    train_dataset = MSDDataset(
        ...
        slice_selection_mode=slice_selection_mode
    )
    ...
    early_stopping_patience = 20  # Adjust as needed
